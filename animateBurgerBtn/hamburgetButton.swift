...
class HamburgerButton : UIButton {
    ...
    func animateWithRatio(rate: Float) {
        let translation = CATransform3DMakeTranslation(0, 0, 0)
        let angle = CGFloat(1.57079 * rate)
        self.top.transform = CATransform3DRotate(translation, angle, 0, 0, 1)

        
        self.middle.strokeStart = 0.1
        self.middle.strokeEnd = 1
    }
}
